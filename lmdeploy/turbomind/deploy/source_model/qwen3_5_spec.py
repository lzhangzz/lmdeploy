...
class Qwen3_5Spec(TextModelSpec):
    ...
    def _build_attention(self, parent, layer, mc, dtype, tp, ranks,
        ...
        # Commit attention linear bundles with standard TP rules
        for name, lin in attn_linears.items():
            if name == 'w_qkv':
                attn._commit_linear('w_qkv', lin, SplitSide.OUTPUT,
                                    model_dtype=dtype)
            elif name == 'wo':
                attn._commit_linear('wo', lin, SplitSide.INPUT,
                                    model_dtype=dtype)
            else:
        ...
        for name, tensor in self.attn_params(layer).items():
            attn._commit_tensor(name, tensor)

        # Norm children (q_norm, k_norm) -- zero-centered
