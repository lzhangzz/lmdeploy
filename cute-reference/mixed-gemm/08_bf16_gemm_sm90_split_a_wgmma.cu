 ...
 *   - Packed buffer stores uint4 (1xuint32 per thread per k_block, 4x smaller)
 *   - Consumer S2R loads 1xuint32, applies unpack_u4_to_bf16 (lop3 + subtract 128)
 *   - Bulk copy transfers 4x less data per stage
 *
 * Target: SM90
 **************************************************************************************************/
...
int main(int argc, char** argv)
{
  ...
  auto run_test = [&](int m, int n, int k, const char* label) -> bool {
    ...
    // Generate A with uint4 values [0,15] encoded in low nibble of BF16 raw bits.
    // The pack function extracts raw_bits & 0xF, so we store val in the low nibble.
    // The GPU dequantizes to bf16(val) via unpack_u4_to_bf16 (lop3 + subtract 128).
    thrust::host_vector<bf16_t> h_A(m * k), h_B(n * k), h_C(m * n, static_cast<bf16_t>(0.0f));
    thrust::host_vector<float> h_A_val(m * k);  // effective uint4 values for CPU reference
    for (int i = 0; i < m * k; ++i) {
      float val = float(rand() % 16);
      h_A_val[i] = val;
      // Encode val in low 4 bits of BF16 raw representation: 0x3F80 = bf16(1.0)
    ...
    // CPU reference: use the dequantized values (uint4 integers 0-15)
    thrust::host_vector<float> h_ref(m * n, 0.0f);
    for (int i = 0; i < m; ++i)
      for (int j = 0; j < n; ++j) {
        float sum = 0.0f;
        for (int l = 0; l < k; ++l)
          sum += h_A_val[i * k + l] * float(h_B[j * k + l]);
    ...
    printf("%s (%dx%dx%d): max error %e -- %s\n", label, m, n, k, max_err, max_err < 2.0f ? "PASS" : "FAIL");
    return max_err < 2.0f;
  };
