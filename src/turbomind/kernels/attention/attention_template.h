...
struct FloatFMA {
    ...
    using ARegisters = T[1];
    using BRegisters = T[1];
    using CRegisters = float[1];

    CUTE_HOST_DEVICE static constexpr void fma(float& d, T const& a, T const& b, float const& c)
    {
        // d = (float)a * (float)b + c;
        d = (float)(a * b) + c;
    }
...
template<class Shape0, class Shape1, class Stride0, class Stride1>
CUTE_HOST_DEVICE constexpr auto reverse(Layout<Shape<Shape0, Shape1>, Stride<Stride0, Stride1>> const& layout)
{
    return make_layout(make_shape(shape<1>(layout), shape<0>(layout)),
                       make_stride(stride<1>(layout), stride<0>(layout)));
}

template<class NewT, class Engine, class Layout>
auto cast(Tensor<Engine, Layout> const& tensor)
{
    auto tmp = make_tensor_like<NewT>(tensor);
    copy(tensor, tmp);
    return tmp;
}
...
struct Attention {
    ...
    using QkComputeType = T;

    using PvAccumType   = float;
    using PvComputeType = T;
    ...
    __device__ static bool thread0()
    {
        return blockIdx.x == 0 && blockIdx.y == 0 && threadIdx.x == 0;
    }

    __device__ Attention(const ParamType& params, SharedStorage& smem, uint8_t* dsmem):
        params_(params),
    ...
    __device__ void Prolugue2()
    {
        using namespace cute;
        using cute::Shape;
        using X = Underscore;

        if (warp_id_ >= kHeadPerCta) {
            return;
        }

        auto bH = Int<kHeadPerCta>{};
        auto bD = Int<kHeadDim>{};

        auto gQ = make_tensor(make_gmem_ptr(&params_.q[batch_idx_ * params_.stride + head_idx_ * kHeadDim]),
                              make_shape(bD, bH));
        auto gK = make_tensor(make_gmem_ptr(&params_.k[batch_idx_ * params_.stride + kv_head_idx_ * kHeadDim]),
                              make_shape(bD, _1{}));
        auto gV = make_tensor(make_gmem_ptr(&params_.v[batch_idx_ * params_.stride + kv_head_idx_ * kHeadDim]),
                              make_shape(bD, _1{}));

        static constexpr int kVecSize = kMaxHeadDim / WARP_SIZE;
        static constexpr int kHeadCnt = (kHeadPerCta + kWarpCount - 1) / kWarpCount;

        auto tQ = make_shape(Int<kVecSize>{}, Int<kHeadCnt>{});
        auto tK = make_shape(Int<kVecSize>{});

        auto cQ = make_identity_tensor(gQ.shape());

        const auto coord = make_coord(lane_id_, warp_id_);

        auto tQgQ = local_tile(gQ, tQ, coord);
        auto tQcQ = local_tile(cQ, tQ, coord);
        auto tKgK = local_tile(gK, tK, lane_id_);
        auto tKgV = local_tile(gV, tK, lane_id_);
        ...
        //     print("tKgV: "), print(tKgK.layout()), print("\n");
        // }

        auto tQrQ = make_tensor_like(tQgQ);
        auto tKrK = make_tensor_like(tKgK);
        auto tKrV = make_tensor_like(tKgV);
        ...
        if (params_.q_bias) {
            auto gQbias = make_tensor(make_gmem_ptr(&params_.q_bias[head_idx_ * kHeadDim]), make_shape(bD, bH));
            auto gKbias = make_tensor(make_gmem_ptr(&params_.k_bias[kv_head_idx_ * kHeadDim]), make_shape(bD, _1{}));
            auto gVbias = make_tensor(make_gmem_ptr(&params_.v_bias[kv_head_idx_ * kHeadDim]), make_shape(bD, _1{}));

            auto tQgQbias = local_tile(gQbias, tQ, coord);
            auto tQgKbias = local_tile(gKbias, tK, lane_id_);
            auto tQgVbias = local_tile(gVbias, tK, lane_id_);

            auto tQrQbias = make_tensor_like(tQgQbias);
            auto tQrKbias = make_tensor_like(tQgKbias);
            auto tQrVbias = make_tensor_like(tQgVbias);

            copy(tQgQbias, tQrQbias);
            copy(tQgQbias, tQrQbias);
            copy(tQgQbias, tQrQbias);
            ...
            PRAGMA_UNROLL
            for (int i = 0; i < size(tKrK); ++i) {
                tKrK(i) += tQrKbias(i);
                tKrV(i) += tQrVbias(i);
            }
        }

        using Frag = Array<T, size<0>(tQrQ)>;

        float rotary_embedding_base =
            params_.rope_theta ? params_.rope_theta[batch_idx_] : params_.rotary_embedding_base;

        // Apply rotary embedding
        // RotaryEmbedding<kVecSize> rotary_emb(
        //     rotary_embedding_base, params_.rotary_embedding_dim, timestep_, {get<0>(tQcQ(0)), 0});

        // PRAGMA_UNROLL
        // for (int s = 0; s < size<1>(tQrQ); ++s) {
        //     rotary_emb.apply(*(Frag*)tQrQ(_, s).data());
        // }
        // rotary_emb.apply(*(Frag*)tKrK.data());

        if (params_.use_logn_attn) {
            LogNScaling logn_scaling(timestep_ + 1, params_.max_position_embeddings);
            PRAGMA_UNROLL
            for (int s = 0; s < size<1>(tQrQ); ++s) {
                logn_scaling.apply(*(Frag*)tQrQ(_, s).data());
            }
        }

        auto sQ = make_tensor(make_smem_ptr(smem_Q_), make_shape(Int<kMaxHeadDim>{}, Int<kHeadPerCta>{}));
        auto sO = make_tensor(make_smem_ptr(smem_O_), make_shape(Int<kMaxHeadDim>{}, Int<kHeadPerCta>{}));
        auto sM = make_tensor(make_smem_ptr(smem_M_), make_shape(Int<kHeadPerCta>{}));
        auto sL = make_tensor(make_smem_ptr(smem_L_), make_shape(Int<kHeadPerCta>{}));

        auto tQsQ = local_tile(sQ, tQ, coord);
        auto tQsO = local_tile(sO, tQ, coord);
        auto tQsM = local_tile(sM, make_shape(Int<kHeadCnt>{}), warp_id_);
        auto tQsL = local_tile(sL, make_shape(Int<kHeadCnt>{}), warp_id_);

        // if (thread0()) {
        //     print("tQsQ: "), print(tQsQ.layout()), print("\n");
        //     print("tQsM: "), print(tQsM.layout()), print("\n");
        // }

        if (kSplitK && step_begin_) {  // Split idx > 0
            if (lane_id_ == 0) {
                fill(tQsM, -std::numeric_limits<float>::infinity());
                clear(tQsL);
            }
            copy(tQrQ, tQsQ);
            clear(tQsO);
        ...
        auto rQ  = make_tensor(tQrQ.data(), reverse(tQrQ.layout()));           // (H, D)
        auto rK  = make_tensor(tKrK.data(), reverse(tKrK.layout()));           // (1, D)
        auto rQK = make_tensor<QkAccumType>(make_shape(size<1>(tQrQ), _1{}));  // (H, 1)

        // if (thread0()) {
        //     print("rQ: "), print(rQ.layout()), print("\n");
        //     print("rK: "), print(rK.layout()), print("\n");
        //     print("rQK: "), print(rQK.layout()), print("\n");
        // }

        gemm(MMA_Atom<MMA_Traits<FloatFMA<T>>>{}, rQ, rK, rQK);  // (H, D) x (1, D) => (H, 1)

        PRAGMA_UNROLL
        for (int mask = WARP_SIZE / 2; mask >= 1; mask /= 2) {
            PRAGMA_UNROLL
            for (int i = 0; i < size(rQK); ++i) {
                rQK(i) += __shfl_xor_sync((uint32_t)-1, rQK(i), mask);
            }
        }
        ...
        copy(tQrQ, tQsQ);
        auto tQrV = make_tensor(tKrV.data(), make_layout(tQrQ.shape(), make_stride(_1{}, _0{})));  // repeat second mode
        // if (thread0()) {
        //     print("tQrV: "), print(tQrV.layout()), print("\n");
        // }
        copy(tQrV, tQsO);

        // rotary_emb.apply(*(Frag*)tKrK.data());

        auto frag_K_store = conv_k_store_(*(Frag*)tKrK.data());
        auto frag_V_store = conv_v_store_(*(Frag*)tKrV.data());
        ...
        auto gK2 = make_tensor(make_gmem_ptr(&k_cache_[block_offset * kHeadDim]), make_shape(bD, _1{}));
        auto gV2 = make_tensor(make_gmem_ptr(&v_cache_[block_offset * kHeadDim]), make_shape(bD, _1{}));

        copy(make_tensor(make_rmem_ptr(&frag_K_store[0]), make_shape(bD)),
             local_tile(gK2, Shape<Int<kVecSize>>{}, lane_id_));
        copy(make_tensor(make_rmem_ptr(&frag_V_store[0]), make_shape(bD)),
             local_tile(gV2, Shape<Int<kVecSize>>{}, lane_id_));
    }

    __device__ static void ApplyRotaryEmbedding(T* k_cache, int seq_len, int head_num, float rotary_embedding_base)
    {
        using namespace cute;
        ...
        k_cache += batch_idx * head_num * seq_len * kHeadDim;

        int warp_id = threadIdx.x / WARP_SIZE;
        int lane_id = threadIdx.x % WARP_SIZE;

        for (int timestep = warp_id; timestep < seq_len; timestep += kWarpCount) {

            auto gK = make_tensor(make_gmem_ptr(&k_cache[head_idx * seq_len * kHeadDim + timestep * kHeadDim]),
                                  make_shape(Int<kHeadDim>{}));

            auto cK = make_identity_tensor(gK.shape());

            static constexpr int kVecSize = kMaxHeadDim / WARP_SIZE;

            auto tK = make_shape(Int<kVecSize>{});

            auto tKgK = local_tile(gK, tK, lane_id);
            auto tKcK = local_tile(cK, tK, lane_id);

            auto tKrK = make_tensor_like(tKgK);

            copy(tKgK, tKrK);

            RotaryEmbedding<kVecSize> rotary_emb(rotary_embedding_base, kHeadDim, timestep, {get<0>(tKcK(0)), 0});

            rotary_emb.apply(*(Array<T, size(tKrK)>*)tKrK.data());
    ...
    __device__ void ComputeSlice(FragmentQ& frag_Q, State& state, const int2& offset, int step, int iter_length)
    {
        ...
        RotaryEmbedding2<kVecKvSize, kKvVecPerThread, MapKv::kWarpAccessC> rotemb(
            rotary_embedding_base, kHeadDim, offset.x);

        int rotemb_offset = step + offset.y - timestep_;
        ...
        // rotemb.apply(state.frag_K_buf[0], rotemb_offset);
        // rotemb_offset += MapKv::kWarpAccessS;
        ...
        for (int _it = 0; _it < iter_length; _it += kKeyPerIter) {
            ...
            for (int si = 0; si < kKvKeyPerThread; ++si) {
                ...
                // rotemb.apply(state.frag_K_buf[next], rotemb_offset);

                // current iter's K fragment
                auto& frag_K = state.frag_K_buf[si % 2];

                // rotemb.apply(frag_K, step + local_offset - timestep_);

                FragmentQ frag_Q2 = frag_Q;
                PRAGMA_UNROLL
                for (int qi = 0; qi < kHeadPerCta; ++qi) {
                    rotemb.apply(frag_Q2.data[qi], -(step + local_offset - timestep_));
                }

                // rotemb_offset += MapKv::kWarpAccessS;
                // if (si == kKvKeyPerThread - 2) {
                //     rotemb_offset += kKeyPerIter - kKvKeyPerThread * MapKv::kWarpAccessS;
                // }

                // warp: (Q, dim) x (WarpThreadS, dim) => (Q, WarpThreadS)
                PRAGMA_UNROLL
                for (int qi = 0; qi < kHeadPerCta; ++qi) {

                    auto qk = qk_dot<QkAccumType, QkComputeType, kThreadPerKey>(frag_Q2.data[qi], frag_K);
    ...
    {
        {
            ...
            // auto a = make_layout(Shape<_16, _16, _8>{}, Stride<_128, _16, _1>{});
            // auto b = zipped_divide(a, Shape<_8, _4, _8>{});

            auto a = make_layout(Shape<_16, _128>{}, Stride<_128, _1>{});
            auto b = zipped_divide(a, Shape<_32>{});

            if (thread0()) {
                // print(a), print('\n');
                // print(b), print('\n');

                // print(Shape<_1, int, _3, int>{100, 0, 200, 0});

                auto layout = Layout<Shape<_16, _128>, Stride<_128, _1>>{};
                auto tile   = make_shape(_4{}, _8{});
                // auto tile = Layout<Shape<_4, _8>>{};

                print(layout), print('\n');
                print(tile), print('\n');
                print(logical_divide(layout, tile)), print('\n');
                print(zipped_divide(layout, tile)), print('\n');
                print(tiled_divide(layout, tile)), print('\n');

                // {
                //     auto tile            = Layout<Shape<_2, _2>, Stride<_1, _2>>{};
                //     auto matrix_of_tiles = Layout<Shape<_3, _4>, Stride<_4, _1>>{};
                //     // print_layout(zipped_product(tile, matrix_of_tiles)), print('\n');
                //     print_layout(logical_product(tile, matrix_of_tiles)), print('\n');
                //     print_layout(blocked_product(tile, matrix_of_tiles)), print('\n');
                //     print_layout(raked_product(tile, matrix_of_tiles)), print('\n');
                //     // print(tiled_product(tile, matrix_of_tiles)), print('\n');
                // }
            }
        }
...
template<typename Attention, typename ParamType = typename Attention::ParamType>
__global__ void attention(ParamType params)
{
    __shared__ typename Attention::SharedStorage shared_storage;

    uint8_t* smem_ptr = dynamic_smem;

    Attention{params, shared_storage, smem_ptr}.Run();
}

template<class Attention>
__global__ void
apply_rotary_embedding(typename Attention::Dtype* k_cache, int seq_len, int head_num, float rotary_embedding_base)
{
    Attention::ApplyRotaryEmbedding(k_cache, seq_len, head_num, rotary_embedding_base);
}

template<typename Attention, typename ParamType = typename Attention::ParamType>
__global__ void attention_reduce(ParamType params)
{
    Attention::Reduce(params);
}
