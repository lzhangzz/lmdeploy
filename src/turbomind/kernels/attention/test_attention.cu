...
#include "../decoder_multihead_attention/kv_cache.h"
#include "attention.h"
...
int main(int argc, char* argv[])
{
    ...
    thrust::universal_vector<half>  k_cache(kBatchSize * KvHeadNum * kSequenceLen * kHeadDim);
    thrust::universal_vector<half>  v_cache(kBatchSize * KvHeadNum * kSequenceLen * kHeadDim);
    ...
    // invokeApplyRotaryEmbedding(k_cache.data().get(), kSequenceLen, kHeadNum, 10000.f, kBatchSize, 0);
    ...
    cudaDeviceSynchronize();
    invokeApplyRotaryEmbedding(k_cache_ref.data().get(), kSequenceLen, kHeadNum, 10000.f, kBatchSize, 0);
    cudaDeviceSynchronize();
