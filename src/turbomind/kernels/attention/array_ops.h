...
template<int N, int V, int C>
struct RotaryEmbedding2 {

    static_assert(N % 2 == 0);

    Array<float, N / 2> inv_freq_[V];

    __device__ RotaryEmbedding2(float base, int dims, int idx)
    {
        ...
        for (int v = 0; v < V; ++v) {
            PRAGMA_UNROLL
            for (int i = 0; i < N / 2; ++i) {
                inv_freq_[v][i] = get_inv_freq(idx + v * C + i * 2, dims, base);
            }
        }
    }

    static __device__ inline float get_inv_freq(int idx, int dims, float base)
    {
        return 1.f / powf(base, idx / (float)dims);
    }

    template<typename T>
    __device__ void apply(Array<T, N> (&x)[V], int timestep)
    {
        ...
        for (int v = 0; v < V; ++v) {
            ...
            for (int i = 0; i < N / 2; ++i) {
                float c, s;
                sincosf((float)timestep * inv_freq_[v][i], &s, &c);
                // float tmp0      = c * (float)x[v][i * 2] - s * (float)x[v][i * 2 + 1];
                // float tmp1      = c * (float)x[v][i * 2 + 1] + s * (float)x[v][i * 2];
                // x[v][i * 2]     = (T)tmp0;
                // x[v][i * 2 + 1] = (T)tmp1;
                T tmp0          = (T)c * x[v][i * 2] - (T)s * x[v][i * 2 + 1];
                T tmp1          = (T)c * x[v][i * 2 + 1] + (T)s * x[v][i * 2];
                x[v][i * 2]     = (T)tmp0;
                x[v][i * 2 + 1] = (T)tmp1;
            }
        }
