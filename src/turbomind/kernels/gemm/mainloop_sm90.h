...
struct MainloopSm90 {
    ...
    __device__ uint64_t get_desc_B(const Tb* smem_B, int iter_k, int iter_n)
    {
        uint32_t  int_ptr  = cast_smem_ptr_to_uint(smem_B);
        const int stride_n = sizeof(Tb) * SmemLayoutB::apply(MMA_Atom::N, 0);
        const int stride_k = sizeof(Tb) * SmemLayoutB::apply(0, MMA_Atom::K);
        int_ptr += stride_n * iter_n + stride_k * iter_k;
        MatrixDescriptor desc;
        desc.bitfield.start               = int_ptr >> 4;
        desc.bitfield.leading_byte_offset = 16 >> 4;
        desc.bitfield.stride_byte_offset  = 64 >> 4;
        desc.bitfield.offset              = 0;
        desc.bitfield.swizzle             = 0;
        return desc.desc;
    }
                               ...
                               SharedStorage& storage)
    {
        ...
        uint64_t                 desc_B[MMA::kTileIterK][MMA::kMmaIterN];
        ...
        auto preload = [&](int k) {
            ...
            for (int n = 0; n < MMA::kMmaIterN; ++n) {
                desc_B[k][n] = get_desc_B(smem_B.pointer, k, n);
            }
        ...
        for (; tile_iter > 0; --tile_iter) {
            ...
            for (int k = 0; k < ITER_K; ++k) {
                ...
                for (int m = 0; m < MMA::kMmaIterM; ++m) {
                    PRAGMA_UNROLL
                    for (int n = 0; n < MMA::kMmaIterN; ++n) {
                        MMA_Atom::fma(frag_C[m][n], frag_A[k][m], desc_B[k][n]);
                    }
                }
